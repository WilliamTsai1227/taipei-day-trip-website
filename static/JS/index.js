...
//     let listBarContent = document.querySelector(".list_bar_content");
//     try {
//         let response = await fetch("http://34.223.129.79:8000/api/mrts");
...
//     let attractions = document.querySelector(".attractions");
//     try {
//         let response = await fetch(`http://34.223.129.79:8000/api/attractions?page=${page}&keyword=${keyword}`);
//         let data = await response.json();
//         page = data.nextPage;
//         for (let i of data.data) {
//             let attraction = document.createElement("div");
...
//         }
//         isLoading = false; // 數據加載完成，將 isLoading 設為 false
//         monitor_attraction_clicks(); // 重新綁定點擊事件
//     } catch (error) {
//         console.log(error);
//         isLoading = false; // 數據加載出錯，將 isLoading 設為 false
//     }
...
//     list_items.forEach(item => {
//         item.addEventListener("click", () => {
//             let id = item.querySelector(".attraction_id").textContent;
//             window.location.replace(`http://34.223.129.79:8000/attraction/${id}`);
//         });
//     });
// }
...
//     await fetch_attractions();
//     monitor_mrt_click();
//     monitor_attraction_clicks(); // 初始加載後綁定一次
//     scroll_list();
//     search();
//     back_to_home_page();
//     scrolling_add_attractions();
